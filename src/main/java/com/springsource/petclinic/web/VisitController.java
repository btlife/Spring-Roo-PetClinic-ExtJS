...
public class VisitController {
    ...
    public ResponseEntity<String> updateFromJson(@RequestBody String json) {
		...
		try {
			Visit record = Visit.fromJsonToVisit(json);
			Visit mergedRecord = (Visit)record.merge();
	        if (mergedRecord == null) {
	            returnStatus = HttpStatus.NOT_FOUND;
				response.setMessage("Visit update failed.");
				response.setSuccess(false);
				response.setTotal(0L);
	        } else {
	            returnStatus = HttpStatus.OK;
				response.setMessage("Visit updated.");
    ...
    public ResponseEntity<String> deleteFromJson(@PathVariable("id") Long id) {
		...
		JsonObjectResponse response = new JsonObjectResponse();
		try {
			Visit record = Visit.findVisit(id);
			record.remove();
            returnStatus = HttpStatus.OK;
			response.setMessage("Visit deleted.");
			response.setSuccess(true);
			response.setTotal(1L);
    ...
    public ResponseEntity<String> listJson() {
		...
		try {
			List<Visit> records = Visit.findAllVisits();
            returnStatus = HttpStatus.OK;
			response.setMessage("All Visits retrieved.");
			response.setSuccess(true);
    ...
    public ResponseEntity<String> createFromJson(@RequestBody String json) {
		...
		try {
			...
			response.setMessage("Visit created.");
			response.setSuccess(true);
			response.setTotal(1L);
			response.setData(record);
		} catch(Exception e) {
